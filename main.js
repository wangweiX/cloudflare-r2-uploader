...
var R2Service = class {
  ...
  constructor(config) {
    ...
    if (!hasS3Credentials) {
      ...
      new import_obsidian3.Notice(`\u672A\u63D0\u4F9B S3 API \u51ED\u8BC1`, 3e3);
    }
    this.s3Client = new S3Client({
      region: "auto",
      // Cloudflare R2 使用 'auto' 作为区域
...
var SettingsTab = class extends import_obsidian7.PluginSettingTab {
  ...
  displayCloudflareR2Settings(containerEl) {
    ...
    new import_obsidian7.Setting(containerEl).setName("Access Key ID").setDesc("S3 API Access Key ID").addText(
      (text) => text.setPlaceholder("\u8F93\u5165 Access Key ID").setValue(this.plugin.settings.r2Settings.accessKeyId || "").onChange(async (value) => {
        this.plugin.settings.r2Settings.accessKeyId = value || void 0;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian7.Setting(containerEl).setName("Secret Access Key").setDesc("S3 API Secret Access Key").addText(
      (text) => text.setPlaceholder("\u8F93\u5165 Secret Access Key").setValue(this.plugin.settings.r2Settings.secretAccessKey || "").onChange(async (value) => {
        this.plugin.settings.r2Settings.secretAccessKey = value || void 0;
        await this.plugin.saveSettings();
      })
    );
  }
